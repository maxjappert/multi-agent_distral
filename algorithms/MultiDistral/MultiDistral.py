...
class MultiDistral:
    ...
    def optimise(self,version):
        ...
        for i in range(iterations):
            ...
            if version==1:
                for j,env in enumerate(self.list_envs):
                    env.reset()
                    agents.append(self.algorithm(self.list_envs[j],num_games,self.turn_limit,self.alpha,self.gamma,self.tau,PSI,BETA,version,self.pi_0_1,self.pi_0_2))
            ...
            if version==1: # 1 distilled policy per agent 
                ...
                new_pi_0_1=total_counts_1/np.maximum(np.apply_over_axes(np.sum, total_counts_1, range(total_counts_1.ndim - 1)), self.eps) #sum over all but last axis
                new_pi_0_2=total_counts_2/np.maximum(np.apply_over_axes(np.sum, total_counts_2, range(total_counts_2.ndim - 1)), self.eps) #sum over all but last axis
                ...
                for j, env in enumerate(self.list_envs):
                    self.list_envs[j].reset()
                    agents.append(
                        self.algorithm(self.list_envs[j], num_games, self.turn_limit, self.alpha, self.gamma, self.tau,
        ...
        plt.plot(list(range(iterations)), p1_total_rewards/num_games)
        plt.plot(list(range(iterations)), p2_total_rewards/num_games)
